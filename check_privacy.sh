...
if [ -f "$hosts_path" ]; then
    blocked=$(grep -c "0.0.0.0" "$hosts_path" 2>/dev/null || true)
    : "${blocked:=0}"
    hosts_size=$(wc -c < "$hosts_path" 2>/dev/null || echo 0)

    # 检查系统 hosts 是否被挂载覆盖
    sys_blocked=$(grep -c "0.0.0.0" /system/etc/hosts 2>/dev/null || true)
    : "${sys_blocked:=0}"
